...
function sortAccountsByLastName(accounts) {
  ...
  const found = accounts.sort((accountA, accountB) => accountA.name.last > accountB.name.last ? 1 : -1);
  return found;
}
...
function getAccountFullNames(accounts) {
  ...
  const found = accounts.map((element) => `${element.name.first} ${element.name.last}`);
  return found;
}
